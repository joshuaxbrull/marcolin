...
const front = `<div class="sheet front"><div class="trim"><div class="photo"><img src="assets/Harley.jpg" alt="Portrait with Harley-Davidson goggles and a black leather jacket"><div class="photo-brand">HARLEY-DAVIDSON<span>EYEWEAR</span></div></div><div class="contact"><div><div class="rule"></div><p class="brand">HARLEY-DAVIDSON<br>EYEWEAR</p></div><div class="person"><h1>MIKE<br>BRULL</h1><p class="title">Brand Specialist</p><p class="phone">301-639-8001</p><p class="email">mbrull@marcolin.com</p></div><div><p class="slogan">Find a shop.<br>Ride there.</p><p class="marcolin">MARCOLIN</p></div></div></div></div>`;
...
const caption = photo => `<div class="intro"><h1>Mike Brull · 4″ × 3″ event card</h1><p>One third contact details, two thirds Harley.jpg portrait. Two back color options use the same regional directory map. Shown with ⅛″ bleed. The supplied photograph provides approximately ${photo.effectivePpi} effective ppi at this size. Approve a physical print and QR scan before the press run.</p><p><a href="proofs/card-light-back.pdf">Light-back PDF</a> · <a href="proofs/card-dark-back.pdf">Dark-back PDF</a> · <a href="proofs/front-trim.png">Front at trim size</a></p></div>`;
const gallery = photo => documentHtml(caption(photo) + `<div class="proofs"><div class="proof"><div class="proof-label">Front · One third / two thirds</div>${front}</div><div class="proof"><div class="proof-label">Back · Light</div>${back(false)}</div><div class="proof"><div class="proof-label">Back · Dark</div>${back(true)}</div></div>`);
...
try {
  ...
  for (const name of ["front", "back-light", "back-dark"]) {
    ...
    if (name === "front") {
      photoMetrics = await page.locator(".photo img").evaluate(img => {
        const { width, height } = img.getBoundingClientRect();
        const factor = Math.max(width / img.naturalWidth, height / img.naturalHeight);
        if (!img.complete || !Number.isFinite(factor) || factor <= 0) throw new Error("The supplied card photograph did not load.");
  ...
  await writeFile("cards/index.html", gallery(photoMetrics));
  ...
  await writeFile("cards/proofs/specification.json", JSON.stringify({ trimInches: [4,3], bleedInches: .125, rasterDpi: 600, frontSplit: { contact: 1/3, photo: 2/3 }, directoryCount: rows.length, directoryIds: rows.map(r=>r.id), qrUrl, qrQuietModules:4, qrErrorCorrection:"Q", photoSource:"cards/assets/Harley.jpg", photoSourcePixels:photoMetrics.sourcePixels, photoEffectivePpi:photoMetrics.effectivePpi, status:"design-proof-requires-physical-print-and-qr-scan" }, null, 2)+"\n");
